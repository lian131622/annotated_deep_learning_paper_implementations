...
class MillScaleDataset(torch.utils.data.Dataset):
    def __init__(self, samplingperiod, N, pca_components=65):
        super().__init__()
        ...
        for item in self._sampleNames:
            ...
            # irf = GaussianFilter_irf_fre(generate_positive_frequency_axis(samplingperiod, 4 * N), irf, 3)
            # # turn into time domain
            # irf = [np.fft.ifft(i).real for i in irf]
            # # put the irf to the return variable
            self._irf.extend(irf)
        ...
        # do PCA
        self._irf = np.array(self._irf)  # turn back to ndarray
        scaler = StandardScaler()
        self._irf = scaler.fit_transform(self._irf)

        # 应用PCA降维
        if pca_components:
            pca = PCA(n_components=pca_components)
            self._irf = pca.fit_transform(self._irf)

        # Turn to tensor
        self._labels = torch.tensor(self._labels, dtype=torch.int)
        self._irf = [torch.tensor(irf, dtype=torch.float32) for irf in self._irf]

    def __len__(self):
        return len(self._irf)

    def __getitem__(self, idx):
        return self._irf[idx], self._labels[idx]
